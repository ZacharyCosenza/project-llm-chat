...
from core.reward import PythonExecutor
from core.dataset import CalculatorDataset
...
class CodeRLDataset(IterableDataset):
    """Online RL dataset mixing calculator, HumanEval, and MBPP problems.

    Each item yields:
      prompt_ids    [T_prompt] LongTensor — BOS + USR + question + ASST
      expected      str | None   — for calculator: ground-truth answer
      test_prefix   str | None   — for HumanEval: function stub prepended to model code
      test_suffix   str | None   — for code tasks: test assertions appended after model code
      problem_type  str          — 'calculator' | 'humaneval' | 'mbpp'

    Train/val splits (mirrors SFT first-shard-as-val convention):
      calculator  Different RNG seeds: train uses seed+0, val uses seed+10_000.
                  Infinite programmatic generation — no index overlap possible.
      humaneval   Index split: first val_fraction of 164 examples → val, rest → train.
                  HumanEval has no official train split; we carve out a fixed holdout.
      mbpp        Official HuggingFace splits: train split → train, test split → val.
    """

    VAL_SEED_OFFSET = 10_000   # added to seed for calculator val stream
    HE_VAL_FRACTION = 0.20     # ~33 of 164 HumanEval examples held out for val

    def __init__(self, weights: dict, tokenizer, split: str = 'train',
        ...
        self._usr = tokenizer.convert_tokens_to_ids('<|user|>')
        self._asst = tokenizer.convert_tokens_to_ids('<|assistant|>')

    def _prompt_ids(self, question: str) -> torch.Tensor:
        return torch.tensor(
            [self._bos, self._usr]
    ...
    @staticmethod
    def _fmt_humaneval(row: dict) -> dict:
        prompt = row['messages'][0]['content']
        return {
            'question':    f"Complete the following Python function:\n\n```python\n{prompt}\n```",
            'expected':    None,
            'test_prefix': prompt,
            'test_suffix': row['test'] + f'\ncheck({row["entry_point"]})',
            'problem_type': 'humaneval',
        }

    @staticmethod
    def _fmt_mbpp(row: dict) -> dict:
        return {
            'question':    f"Write a Python function to solve the following:\n\n{row['text']}",
            'expected':    None,
            'test_prefix': None,
            'test_suffix': '\n'.join(row['test_list']),
            'problem_type': 'mbpp',
    ...
    def __iter__(self):
        ...
        # HumanEval: index-based split (no official train split exists)
        he_path = Path('data/humaneval_data/test.parquet')
        if not he_path.exists():
            raise FileNotFoundError(
                f'HumanEval not found: {he_path}\n'
                'Run: python -m core.dataset --humaneval'
            )
        all_he = pd.read_parquet(he_path).to_dict('records')
        n_val_he = max(1, int(len(all_he) * self.HE_VAL_FRACTION))
        he_pool = all_he[:n_val_he] if self.split == 'val' else all_he[n_val_he:]

        # MBPP: official train/test split
        mbpp_split = 'test' if self.split == 'val' else 'train'
        mbpp_path = Path(f'data/mbpp_data/{mbpp_split}.parquet')
        if not mbpp_path.exists():
            raise FileNotFoundError(
                f'MBPP not found: {mbpp_path}\n'
                'Run: python -m core.dataset --mbpp'
            )
        mbpp_pool = pd.read_parquet(mbpp_path).to_dict('records')

        # Calculator: offset seed to guarantee non-overlapping problem streams
        calc_seed = self.seed + (self.VAL_SEED_OFFSET if self.split == 'val' else 0)
        calc_iter = iter(CalculatorDataset(seed=calc_seed + self.rank))

        rng = random.Random(self.seed + self.rank)
        sources = list(self.weights.keys())
        weights = [self.weights[s] for s in sources]
        he_idx, mbpp_idx = self.rank, self.rank
        ...
        while True:
            ...
            if src == 'calculator':
                ...
                base = {
                    ...
                    'test_prefix': None,
                    'test_suffix': None,
                    'problem_type': 'calculator',
                }
            elif src == 'humaneval':
                base = self._fmt_humaneval(he_pool[he_idx % len(he_pool)])
                he_idx += self.world_size
            else:  # mbpp
                base = self._fmt_mbpp(mbpp_pool[mbpp_idx % len(mbpp_pool)])
                mbpp_idx += self.world_size

            base['prompt_ids'] = self._prompt_ids(base['question'])
            yield base
...
def compute_rl_loss(logits, labels, loss_mask, returns):
    """REINFORCE loss: per-token CE weighted by discounted returns, masked to trained positions.

    Mirrors compute_loss — logits[t] predicts labels[t] (pre-shifted by caller).
    loss_mask: float [T] — 1.0 for positions contributing to the loss (e.g. assistant tokens)
    returns:   float [T] — per-token discounted returns
    """
    per_token = F.cross_entropy(
        logits.view(-1, logits.size(-1)), labels.view(-1), reduction='none'
    ).view(labels.shape)
    return (per_token * loss_mask * returns).sum() / loss_mask.sum().clamp(min=1)
...
def _optimizer_step(scaler, optimizer, model, gradient_clip_val, scheduler):
    """Unscale → clip → step → zero_grad → scheduler step. Shared across all modes."""
...
def _validate_and_checkpoint(validate_fn, model, global_step, checkpoint_dir, rank, world_size):
    """Run validate_fn, restore train mode, save + sync checkpoint."""
    validate_fn()
    model.train()
    if checkpoint_dir:
...
def train_epoch(model, train_loader, optimizer, scheduler, scaler, device, rank, world_size,
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if (batch_idx + 1) % accumulate_grad_batches == 0:
            _optimizer_step(scaler, optimizer, model, gradient_clip_val, scheduler)
            global_step += 1
            ...
            if global_step % log_every_n_steps == 0:
                ...
                if rank == 0:
                    ...
                    log_dict = {
                        ...
                        'lr': current_lr,
                        'opt/phase': 0 if global_step < warmup_steps else 1,
            ...
            if global_step % val_check_interval == 0:
                _validate_and_checkpoint(
                    lambda: validate(model, val_loaders, device, rank, world_size,
                                     global_step, tokenizer, use_wandb, limit_val_batches),
                    model, global_step, checkpoint_dir, rank, world_size,
                )
...
def train_dpo_epoch(model, train_loaders, dataset_weights, optimizer, scheduler, scaler,
    ...
    while global_step < max_steps:
        ...
        if (batch_idx + 1) % accumulate_grad_batches == 0:
            ...
            if global_step % log_every_n_steps == 0:
                loss_reduced = reduce_tensor(torch.tensor(loss.item() * accumulate_grad_batches, device=device), world_size)
            ...
            if global_step % val_check_interval == 0:
                _validate_and_checkpoint(
                    lambda: validate_dpo(model, val_loaders, device, rank, world_size,
                                         global_step, tokenizer, dpo_beta, use_wandb, limit_val_batches),
                    model, global_step, checkpoint_dir, rank, world_size,
                )
...
def truncate_at_eos(ids: torch.Tensor, eos_id: int) -> torch.Tensor:
    """Return ids[:first_eos+1], or ids as-is if no EOS found."""
    pos = (ids == eos_id).nonzero(as_tuple=True)[0]
    return ids[:pos[0] + 1] if len(pos) > 0 else ids


def discounted_returns(advantage: float, n_tokens: int, gamma: float,
                       device: torch.device) -> torch.Tensor:
    """Per-token discounted return for a terminal reward.

    Token t gets gamma^(n_tokens-1-t) * advantage.
    Last token receives the full advantage; earlier tokens receive less.
    """
    exponents = torch.arange(n_tokens - 1, -1, -1, dtype=torch.float, device=device)
    return advantage * (gamma ** exponents)


def train_code_epoch(model, dataset, executor, optimizer, scheduler, scaler,
                     device, rank, world_size, global_step, max_steps,
                     batch_size, accumulate_grad_batches, gradient_clip_val,
                     log_every_n_steps, use_wandb, pbar, val_check_interval,
                     val_dataset, tokenizer, limit_val_batches, gamma,
                     checkpoint_dir=None):
    model.train()
    optimizer.zero_grad()
    base_model = model.module if isinstance(model, DDP) else model
    ...
    while global_step < max_steps:
        items = [next(data_iter) for _ in range(batch_size)]

        prompt_ids_list, gen_ids_list, loss_masks_list, t_gen_list, rewards = [], [], [], [], []
        with torch.no_grad():
            for item in items:
                pid = item['prompt_ids'].to(device)
                full_ids = base_model.predict(pid.unsqueeze(0), executor.max_new_tokens,
                                              temperature=0.8, top_k=50).squeeze(0)
                gen_ids = truncate_at_eos(full_ids[pid.shape[0]:], tokenizer.eos_token_id)
                response = tokenizer.decode(gen_ids, skip_special_tokens=True)
                reward, gen_ids_padded, loss_mask = executor(pid, gen_ids, response,
                                                             expected=item.get('expected'),
                                                             test_prefix=item.get('test_prefix'),
                                                             test_suffix=item.get('test_suffix'))
                prompt_ids_list.append(pid)
                gen_ids_list.append(gen_ids_padded)
                loss_masks_list.append(loss_mask)
                t_gen_list.append(gen_ids.shape[0])
        ...
        item_losses = []
        for pid, gen_ids, loss_mask, T_gen, adv in zip(
                prompt_ids_list, gen_ids_list, loss_masks_list, t_gen_list, advantages):
            if T_gen == 0:
                continue
            full_ids = torch.cat([pid, gen_ids])
            returns = torch.zeros_like(loss_mask)
            returns[pid.shape[0] - 1 : pid.shape[0] - 1 + T_gen] = discounted_returns(adv.item(), T_gen, gamma, device)

            with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16,
                                    enabled=(device.type == 'cuda')):
                logits = model(full_ids.unsqueeze(0)).squeeze(0)

            item_losses.append(compute_rl_loss(logits[:-1], full_ids[1:], loss_mask, returns))

        if not item_losses:
            step_count += 1
            continue

        loss = torch.stack(item_losses).mean() / accumulate_grad_batches
        scaler.scale(loss).backward()
        step_count += 1
        ...
        if step_count % accumulate_grad_batches == 0:
            ...
            if global_step % log_every_n_steps == 0:
                loss_red = reduce_tensor(torch.tensor(loss.item() * accumulate_grad_batches, device=device), world_size)
                if rank == 0:
                    mean_reward = rewards_t.mean().item()
                    exec_rate = sum(1 for r in rewards if r > 0) / len(rewards)
                    log_dict = {
                        'train/rl_loss': loss_red.item(),
                        'train/mean_reward': mean_reward,
                        'train/exec_rate': exec_rate,
            ...
            if global_step % val_check_interval == 0:
                _validate_and_checkpoint(
                    lambda: validate_code(model, val_dataset, executor, device, rank,
                                          world_size, global_step, tokenizer, use_wandb,
                                          limit_val_batches),
                    model, global_step, checkpoint_dir, rank, world_size,
                )
...
def validate_code(model, val_dataset, executor, device, rank, world_size,
                  global_step, tokenizer, use_wandb, limit_val_batches):
    """Code RL validation: mean reward + exec rate per problem type, then qualitative evals."""
    model.eval()
    base_model = model.module if isinstance(model, DDP) else model
    log_dict = {'global_step': global_step}
    rewards_by = defaultdict(list)
    ...
    with torch.no_grad():
        for _ in range(limit_val_batches):
            ...
            gen_ids = truncate_at_eos(full_ids[pid.shape[0]:], tokenizer.eos_token_id)
            response = tokenizer.decode(gen_ids, skip_special_tokens=True)
            reward, _, _ = executor(pid, gen_ids, response,
                                    expected=item.get('expected'),
                                    test_prefix=item.get('test_prefix'),
                                    test_suffix=item.get('test_suffix'))
            rewards_by[item['problem_type']].append(reward)

    for ptype, rs in rewards_by.items():
        mean_r = sum(rs) / len(rs) if rs else 0.0
        exec_rate = sum(1 for r in rs if r > 0) / len(rs) if rs else 0.0
        log_dict[f'val/{ptype}/mean_reward'] = mean_r
        log_dict[f'val/{ptype}/exec_rate'] = exec_rate
        print0(f"  val/{ptype} - mean_reward: {mean_r:.4f}, exec_rate: {exec_rate:.3f}")
    ...
    if rank == 0:
        ...
        if use_wandb:
            wandb.log(log_dict)
            if sentence_completions:
                wandb.log({"sentence_completions": wandb.Table(
                    columns=["step", "prompt", "completion"],
                    data=[[global_step, i['prompt'], i['completion']] for i in sentence_completions],
                )})
            if conversation_completions:
                wandb.log({"conversation_validation": wandb.Table(
                    columns=["step", "group", "desc", "system", "user", "response"],
                    data=[[global_step, i['group'], i['desc'], i['system'], i['user'], i['response']]
                          for i in conversation_completions],
                )})
...
def run(args):
    ...
    if is_code:
        executor = PythonExecutor(pad_token_id=tokenizer.pad_token_id)
        code_dataset = CodeRLDataset(train_config['datasets'], tokenizer,
                                     split='train', seed=42, rank=rank, world_size=world_size)
        code_val_dataset = CodeRLDataset(train_config['datasets'], tokenizer,
                                         split='val', seed=42, rank=0, world_size=1)
        val_loaders = {}  # unused; validate_code handles its own eval
    elif is_dpo:
    ...
    if use_wandb:
        ...
        if is_dpo:
            ...
            if is_code:
                wandb_config['max_new_tokens'] = executor.max_new_tokens
                wandb_config['exec_timeout'] = executor.timeout
                wandb_config['gamma'] = train_config['gamma']
            else:
    ...
    desc = "Code RL" if is_code else ("DPO Training" if is_dpo else "Training")
    pbar = tqdm(total=max_steps, initial=global_step, disable=(rank != 0), desc=desc)

    while global_step < max_steps:
        if is_code:
            global_step = train_code_epoch(
                model, code_dataset, executor, optimizer, scheduler, scaler,
                device, rank, world_size, global_step, max_steps,
                batch_size, accumulate_grad_batches, gradient_clip_val,
                log_every_n_steps, use_wandb, pbar, val_check_interval,
                code_val_dataset, tokenizer, limit_val_batches,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='dpo', choices=[
        ...
        'sft', 'sft-lima', 'sft-benchmark', 'sft-topic-switch', 'dpo', 'code',
    ])
