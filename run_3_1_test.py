...
class LightningGPT(pl.LightningModule):
    ...
    def train_dataloader(self):
        if self._train_loader is None:
            ...
            # Use random data generator instead of tokenizing_dataloader
            self._train_loader = self._create_random_train_dataloader(device)
        return self._train_loader

    def val_dataloader(self):
        if self._val_loader is None:
            device = "cuda" if self.device.type == "cuda" else "cpu"
            # Use random data generator instead of simple_dataloader
            self._val_loader = self._create_random_val_dataloader(device)
        return self._val_loader

    def _create_random_train_dataloader(self, device):
        """Create a dataloader that yields random token sequences for training."""
        class RandomTrainDataset:
            def __init__(self, vocab_size, batch_size, max_seq_len, device):
            ...
            def __iter__(self):
                while True:
                    ...
                    x = torch.randint(0, self.vocab_size, (self.batch_size, self.max_seq_len), device=self.device)
                    # Generate random target tokens
                    y = torch.randint(0, self.vocab_size, (self.batch_size, self.max_seq_len), device=self.device)
                    # Add dummy metadata for compatibility
                    metadata = torch.zeros(self.batch_size, dtype=torch.long, device=self.device)
                    yield (x, y, metadata)

        return RandomTrainDataset(self.hparams.vocab_size, self.batch_size, self.max_seq_len, device)

    def _create_random_val_dataloader(self, device):
        """Create a dataloader that yields random token sequences for validation."""
        class RandomValDataset:
            def __init__(self, vocab_size, batch_size, max_seq_len, device):
                self.vocab_size = vocab_size
                self.batch_size = batch_size
                self.max_seq_len = max_seq_len
        ...
        return RandomValDataset(self.hparams.vocab_size, self.batch_size, self.max_seq_len, device)
