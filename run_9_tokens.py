...
def create_dataloaders(data_dir, tokenizer, batch_size, seq_length,
                       num_workers, val_sequences, rank, world_size, bos_token_id=None):
    all_files = sorted(Path(data_dir).glob("*.parquet"))
    val_files = all_files[:1]
    train_files = all_files[1:]
    print0(f"Data split: {len(train_files)} train shards, {len(val_files)} val shards")

    train_dataset = StreamingParquetDataset(
        train_files, tokenizer, seq_length, rank=rank, world_size=world_size,
    ...
    val_dataset = StreamingParquetDataset(
        val_files, tokenizer, seq_length, rank=rank, world_size=world_size,
...
def train_epoch(model, train_loader, optimizer, scheduler, scaler, device, rank, world_size,
                ...
                warmup_steps, checkpoint_dir=None, core_examples=27, pad_token_id=None):
    model.train()
    optimizer.zero_grad()
        ...
        if (batch_idx + 1) % accumulate_grad_batches == 0:
            ...
            if global_step % val_check_interval == 0:
                validate(model, val_loader, device, rank, world_size, global_step,
                        tokenizer, use_wandb, limit_val_batches, core_examples, pad_token_id)
                model.train()
...
def validate(model, val_loader, device, rank, world_size, global_step, tokenizer,
             use_wandb, limit_val_batches, core_examples=27, pad_token_id=None):
    model.eval()
    total_loss = 0.0
    num_batches = 0
    ignore_index = pad_token_id if pad_token_id is not None else -100
...
if __name__ == "__main__":
    ...
    train_loader, val_loader = create_dataloaders(
        'data/base_data', tokenizer, batch_size, max_seq_len,
